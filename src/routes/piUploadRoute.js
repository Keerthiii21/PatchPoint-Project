...
// Used by Raspberry Pi for automated pothole uploads
// Accepts multipart/form-data: image (file), lat, lon, depth, timestamp, confidence (all optional except image)
router.post('/pi-upload', upload.single('image'), async (req, res) => {
  try {
    const { lat, lon, depth, timestamp, confidence } = req.body;
    ...
    const uploadStream = cloudinary.uploader.upload_stream({ folder: 'patchpoint/pi' }, async (error, result) => {
      ...
      // Create pothole record with Pi-provided metadata
